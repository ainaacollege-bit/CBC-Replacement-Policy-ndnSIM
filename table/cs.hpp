...
public: // enumeration
  ...
  void 
  writeInfo(const Name& prefix, std::string event) const
  {
    std::ofstream out("resultRun/eviction-rate/cs-content-cache-trace.txt", std::ios::app);
    //ns3::Time time = ns3::Simulator::Now().GetSeconds();
    /**
    if (status == true)
        totalHits++;
    else
    ...
    if (ns3::Simulator::GetContext() < 1000) { // this check prevents 4535459 value of Context
      ...
      out << ns3::Simulator::Now().GetSeconds() << "\t"
                << prefix << "\t"
                << event << std::endl;
      
      
      // interest
      /**out << time.ToDouble(ns3::Time::S) << "\t"
              << node->GetId() << "\t"
              << prefix << "\t"
              << status << "\t"
              << totalHits << "\t"
...
private:
  ...
  mutable int totalHits = 0;
  mutable int totalMisses = 0;
};
